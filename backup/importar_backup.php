...
<?php
    ...
    if (isset($_FILES['xmlcliente'])){
        ...
        foreach($xml as $cliente) {
            $sql = "INSERT INTO cliente (codigo, nome, cpf, email, fone_com, fone_res, celular, endereco, bairro, cidade, estado, cep)
                VALUES ('$cliente->codigo','$cliente->nome', '$cliente->cpf', '$cliente->email','$cliente->fone_com','$cliente->fone_res',
    ...
    if (isset($_FILES['xmlfuncionario'])){
        ...
        foreach($xml as $funcionario){
            $sql = "INSERT INTO funcionario (codigo, nome, apelido, cargo, setor, cpf, rg, ctps, data_nasc, estado_civil, numero_filhos, tipo_sanguineo, email, fone_com,
                                fone_res, celular, escolaridade, situacao, ano, endereco, bairro,  login, senha, nivel)
                VALUES ('$funcionario->codigo','$funcionario->nome', '$funcionario->apelido', '$funcionario->cargo','$funcionario->setor','$funcionario->cpf',
    ...
    if (isset($_FILES['xmlos'])){
        ...
        foreach($xml as $os){
            $sql = "INSERT INTO ordemservico (Cod_Equipamento, Funcionario, Data_Entrada, Hora_Entrada, Cliente, Equipamento, Modelo, Marca, Patrimonio, Serie, Setor, Garantia, Problemacliente, Data_Agenda, Hora_Agenda, DiagnosticoTecnico, Solucao, Previsaoentrega, Dataentrega, Recebido, Arquivo, valor)
                VALUES ('$os->Cod_Equipamento','$os->Funcionario', '$os->Data_Entrada', '$os->Hora_Entrada','$os->Cliente','$os->Equipamento',
                        '$os->Modelo','$os->Marcao','$os->Patrimonio','$os->Serie','$os->Setor', '$os->Garantia', '$os->Problemacliente',
